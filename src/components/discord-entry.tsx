...
const discord = new DiscordSDK(process.env.NEXT_PUBLIC_DISCORD_CLIENT_ID);

export default function DiscordEntry() {
  const [auth, setAuth] = useState<null | Awaited<
    ReturnType<typeof discord.commands.authenticate>
  >>(null);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setAuth(auth);
        const channel = await discord.commands.getChannel({
